...
describe("User follows requests", () => {
  it("signs up 3 users, which follow and unfollow each other", async () => {
    const userData1 = await app
      .handle(
        new Request("localhost:3000/api/users", {
          method: "POST",
    ...
    const profiles1 = await Promise.all(
      ["jason", "bob"].map((username) =>
        app
          .handle(
            new Request(`localhost:3000/api/profiles/${username}/follow`, {
              method: "POST",
              headers: {
    ...
    const profile3 = await app
      .handle(
        new Request("localhost:3000/api/profiles/michael/follow", {
          method: "POST",
          headers: {
            "content-type": "application/json",
    ...
    const profile4 = await app
      .handle(
        new Request("localhost:3000/api/profiles/jason/follow", {
          method: "DELETE",
          headers: {
            "content-type": "application/json",
            authorization: `Token ${tokenMichael}`
    ...
    const profile5 = await app
      .handle(
        new Request("localhost:3000/api/profiles/michael", {
          method: "GET",
          headers: {
            "content-type": "application/json",
            authorization: `Token ${tokenJason}`
